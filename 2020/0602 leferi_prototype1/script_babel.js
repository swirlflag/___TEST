...
const ScrollArea = (_temp = class ScrollArea {
  constructor(option) {
    _defineProperty(this, "types", ['circle', 'rect', 'triangle', 'oval']);
  ...
  addSection(section) {
    ...
    this.wrap.appendChild(el_section); // const el_particles = el_section.querySelectorAll('.particle_item');
...
const test = () => {
  ...
  for (let i = 0; i < items.length; ++i) {
    ...
    if (0 < elementSt && elementSt < end) {
      const percent = elementSt / end * 100; // item.setAttribute('data-target-y', 'hey');
